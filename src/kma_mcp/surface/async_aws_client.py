...
class AsyncAWSClient:
    ...
    async def get_minutely_data(
        ...
        Example:
            >>> async with AsyncAWSClient('your_auth_key')
            >>> ...     data = await client.get_minutely_data('202501011200')
            >>> # Or using datetime
            >>> from datetime import datetime
            >>> ...     data = await client.get_minutely_data(datetime(2025, 1, 1, 12, 0))
        """
        if isinstance(tm, datetime):
            tm = tm.strftime('%Y%m%d%H%M')

        params = {'tm': tm, 'stn': str(stn), 'help': '0'}
        return await self._make_request('kma_aws.php', params)
    ...
    async def get_minutely_period(
        ...
        Example:
            >>> async with AsyncAWSClient('your_auth_key')
            >>> ...     data = await client.get_minutely_period('202501011200', '202501011300')
        """
