...
export class SnowClient extends BaseKMAClient {
  ...
  /**
   * @deprecated This endpoint (kma_sd.php) is not documented in the official API.
   * Consider using getSnowDepth() with the documented kma_snow1.php endpoint.
   *
   * Get hourly snow depth observation data for a single time (undocumented endpoint)
   */
  async getHourlyData(tm: string | Date, stn: number | string = 0): Promise<SnowObservation[]> {
    const timeStr = typeof tm === 'string' ? tm : this.formatDateTime(tm);
    return this.makeRequest<SnowObservation>('kma_sd.php', {
      tm: timeStr,
      stn: String(stn),
    });
  }
  ...
  /**
   * @deprecated This endpoint (kma_snow_2.php) is not documented in the official API.
   * Consider using getSnowPeriod() with the documented kma_snow2.php endpoint.
   *
   * Get hourly snow depth observation data for a time period (undocumented endpoint)
   */
  async getHourlyPeriod(
    ...
    return this.makeRequest<SnowObservation>('kma_snow_2.php', {
      tm1: time1Str,
      tm2: time2Str,
      stn: String(stn),
    });
  ...
  /**
   * @deprecated This endpoint is not documented in the official API.
   *
   * Get daily snow depth observation data for a single day (undocumented endpoint)
   */
  async getDailyData(tm: string | Date, stn: number | string = 0): Promise<SnowObservation[]> {
    const dateStr = typeof tm === 'string' ? tm : this.formatDateTime(tm, false);
    return this.makeRequest<SnowObservation>('kma_sd_day.php', {
      tm: dateStr,
      stn: String(stn),
    });
  }
  ...
  /**
   * @deprecated This endpoint (kma_snow_day2.php) is not documented in the official API.
   *
   * Get daily snow depth observation data for a date range (undocumented endpoint)
   */
  async getDailyPeriod(
    ...
    return this.makeRequest<SnowObservation>('kma_snow_day2.php', {
      tm1: date1Str,
      tm2: date2Str,
      stn: String(stn),
    });
