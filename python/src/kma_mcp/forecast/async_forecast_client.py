...
class AsyncForecastClient:
    ...
    async def get_short_term_forecast(
        ...
        Example:
            ...
            >>> ...     data = await client.get_short_term_forecast('202501011200')
        """
        params = {'tm_fc': tm_fc, 'stn': str(stn), 'help': '0'}
        return await self._make_request('kma_sfcfct.php', params)
    ...
    async def get_medium_term_forecast(
        ...
        Example:
            >>> async with AsyncForecastClient('your_auth_key')
            >>> ...     data = await client.get_medium_term_forecast('202501011200')
        """
        params = {'tm_fc': tm_fc, 'stn': str(stn), 'help': '0'}
        return await self._make_request('kma_mtfcst.php', params)
    ...
    async def get_weekly_forecast(
        ...
        Example:
            >>> async with AsyncForecastClient('your_auth_key')
            >>> ...     data = await client.get_weekly_forecast('202501011200')
        """
        params = {'tm_fc': tm_fc, 'stn': str(stn), 'help': '0'}
        return await self._make_request('kma_wkfcst.php', params)
